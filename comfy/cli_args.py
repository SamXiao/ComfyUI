...
parallel_group.add_argument("--parallel-execution", dest="parallel_execution", action="store_true", default=None, help="Run one prompt worker per visible GPU so independent prompts execute in parallel. Enabled by default when more than one GPU is visible.")
parallel_group.add_argument("--no-parallel-execution", dest="parallel_execution", action="store_false", help="Disable per-GPU parallel prompt execution and run a single serial worker even with multiple GPUs.")
parser.add_argument("--parallel-workers", type=int, default=None, metavar="N", help="Number of parallel prompt workers. Defaults to one per visible GPU. If N is less than the GPU count only the first N GPUs are used; if greater, workers share GPUs (bounded by VRAM).")
